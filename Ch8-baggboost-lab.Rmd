...
This is a simulated data set containing sales of child car seats at 400 different stores.
...
Use as predictors all variables except Sales.
...
# how should we plot the cross-validation error?

```

```{r}
# plot size vs. k
plot(cv.carseats$k, cv.carseats$size)
...
medv: median value of owner-occupied homes in $1000s (we need to predict this).
...
The square root of the MSE is therefore around $5.941$, indicating that this model leads to test predictions that are (on average) within approximately $5941$ of the true median home value for the census tract.
...
# try some of the other parameters of the function
#   ?randomForest
